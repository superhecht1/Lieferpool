...
// GET /api/pools – alle offenen Pools
...
router.get('/:id', auth, async (req, res) => {
  try {
    ...
    const { rows: commitments } = await db.query(`
      ...
      FROM commitments cm
      JOIN erzeuger e ON e.id = cm.erzeuger_id
      WHERE cm.pool_id = $1
...
router.post('/', auth, role('caterer', 'admin'), async (req, res) => {
  ...
  try {
    // Caterer-ID ermitteln
    const { rows: [caterer] } = await db.query(
      `SELECT id FROM caterer WHERE user_id = $1`,
      [req.user.id]
    );
...
router.post('/:id/commit', auth, role('erzeuger'), async (req, res) => {
  ...
  try {
    ...
    if (!erzeuger) {
      return res.status(403).json({ error: 'Kein verifiziertes Zertifikat vorhanden' });
    }

    // Commitment anlegen
    const { rows: [commitment] } = await client.query(`
      INSERT INTO commitments (pool_id, erzeuger_id, menge)
      VALUES ($1, $2, $3)
      RETURNING *
    `, [pool.id, erzeuger.id, menge]);

    // Pool-Füllstand aktualisieren
    ...
    // On-chain Commitment registrieren
